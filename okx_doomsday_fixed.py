...
class Config:
    ...
    trailing_takeprofit_activation_pct: float = 0.03  # 移动止盈触发涨幅3%
...
class OptimizedSignalGenerator:
    ...
    def get_dynamic_strength_threshold(self, atr_pct: float) -> float:
        """
        根据市场波动率动态调整信号强度阈值
        
        参数:
            atr_pct: ATR百分比，表示市场波动率
            
        返回:
            动态信号强度阈值
        """
        if atr_pct > 0.10:  # 高波动市场（>10%）
            return 18  # 更严格，避免噪音交易
        elif atr_pct < 0.03:  # 低波动市场（<3%），当前状态
            return 5  # 更宽松，捕捉低波动机会（从10降低到5以增加交易频率）
        else:  # 正常波动（3%-10%）
            return 15  # 标准值
    ...
    def get_position_size_multiplier(self, signal_strength: float) -> float:
        ...
        参数:
            signal_strength: 信号强度分数（0-100）
            
        返回:
            仓位乘数（0-1.0）
        """
        if signal_strength >= 18:  # 高质量信号
            return 1.0  # 标准仓位
        elif signal_strength >= 15:  # 良好信号
            return 0.9  # 90%仓位（从0.8提高到0.9）
        elif signal_strength >= 12:  # 中等信号
            return 0.7  # 70%仓位（从0.5提高到0.7）
        elif signal_strength >= 10:  # 合格信号
            return 0.5  # 50%仓位（从0.3提高到0.5）
        elif signal_strength >= 5:   # 低质量信号（新增，用于低波动市场）
            return 0.30 # 30%仓位（从0.15提高到0.30）
        else:  # 低于最低阈值
            return 0.0  # 不交易
    ...
    def _generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        long_condition1 = (
            ...
            (df['rsi'] > 30) & (df['rsi'] < self.config.rsi_overbought) &
        ...
        short_condition1 = (
            ...
            (df['momentum_acc'] < 0) &
            (df['volume_ratio'] > self.config.min_volume_ratio) &
            (df['rsi'] > self.config.rsi_oversold) & (df['rsi'] < 75) &
            (df['atr_pct'] < self.config.max_atr_pct)
        )
    ...
    def _calculate_signal_strength(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            if row['signal'] in ['long', 'short']:
                ...
                if scores:
                    df.at[idx, 'signal_strength'] = np.average(scores, weights=weights[:len(scores)])
        
        # 清理临时列
        temp_cols = ['momentum_score', 'volume_score', 'rsi_score', 'trend_score', 'bb_score', 'volatility_score']
    ...
    def _filter_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 计算动态强度阈值（基于平均ATR波动率）
        if len(df) > 0 and 'atr_pct' in df.columns:
            avg_atr_pct = df['atr_pct'].mean()
            dynamic_threshold = self.get_dynamic_strength_threshold(avg_atr_pct)
        else:
            avg_atr_pct = 0.0
            dynamic_threshold = 15  # 默认值
        
        # 强度过滤（动态阈值）
        strength_filtered = (df['signal'] != 'hold') & (df['signal_strength'] < dynamic_threshold)
        ...
        if original_signals > 0:
            ...
            if 'signal_strength' in df.columns:
                for strength in [0, 10, 12, 15, 18, 20]:
                    count = ((df['signal'] != 'hold') & (df['signal_strength'] >= strength)).sum()
                    strength_distribution[f'≥{strength}'] = count
            ...
            self.logger.info(
                f"动态阈值: {dynamic_threshold:.1f}分 (平均ATR: {avg_atr_pct:.2%}), "
                f"过滤{strength_filtered_count}个强度<{dynamic_threshold:.1f}信号, 过滤率{filtered_pct:.1f}%"
            )
            if strength_distribution:
                self.logger.info(f"信号强度分布: {strength_distribution}")
        else:
    ...
    def _calculate_position_size(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            if row['signal'] in ['long', 'short'] and row['signal_strength'] > 0:
                # 获取层级仓位乘数
                strength_multiplier = self.get_position_size_multiplier(row['signal_strength'])
                
                if strength_multiplier == 0.0:
                    # 信号强度低于最低阈值，不交易
                    df.at[idx, 'signal'] = 'hold'  # 标记为不交易
                    df.at[idx, 'position_size'] = 0.0
                    continue
                ...
                if 'atr_pct' in row:
                    self.logger.debug(
                        f"层级仓位: 强度{row['signal_strength']:.1f}分 → 乘数{strength_multiplier:.1f}, "
                        f"ATR调整{atr_multiplier:.2f}, 最终仓位{position_size*100:.1f}%"
                    )
...
class FixedTradingSystem:
    ...
    def __init__(self, config: Config):
        ...
        # 移动止盈跟踪
        self.trailing_takeprofit_activated = False  # 移动止盈是否已激活
        self.dynamic_takeprofit_price = 0  # 动态止盈价格
    ...
    def set_leverage(self):
        """设置杠杆 - 修复版，确保正确传递marginMode和tdMode参数"""
        ...
        methods = [
            ...
            lambda: self.exchange.set_leverage(
                self.config.leverage, 
                self.config.symbol,
                {'marginMode': self.config.margin_mode, 'tdMode': self.config.td_mode}
            ),
            # 方法3: 使用关键字参数
            lambda: self.exchange.set_leverage(
                self.config.leverage, 
            ...
            # 方法4: 最简单方式（回退）
            lambda: self.exchange.set_leverage(self.config.leverage, self.config.symbol),
        ]
        ...
        # 所有方法都失败
        self.logger.warning(f"⚠️ 所有杠杆设置方法均失败，使用交易所默认杠杆(可能为3x)")
        self.logger.warning(f"   建议手动在OKX账户中设置{self.config.leverage}x杠杆")
        return False
    ...
    def calculate_dynamic_stop_loss_take_profit(self, current_price: float, signal_type: str, atr_value: float = None) -> Tuple[float, float]:
        """计算动态止损止盈价格（增强版：支持ATR动态调整）"""
        # 基础百分比止损止盈
        ...
        self.logger.info(f"风控价格: {signal_type} 止损={stop_loss_price:.2f}({stop_loss_pct:.2%}), 止盈={take_profit_price:.2f}({take_profit_pct:.2%})")
        ...
        try:
            ...
            if order.get('id'):
                self.set_stop_loss_take_profit(
                    order['id'], current_price, signal_type
                )
    ...
    def set_stop_loss_take_profit(self, order_id: str, current_price: float, signal_type: str, atr_value: float = None):
        """后设置止损止盈"""
        try:
            # 计算止损止盈价格（支持ATR动态调整）
            stop_loss_price, take_profit_price = self.calculate_dynamic_stop_loss_take_profit(
                current_price, signal_type, atr_value
            )
            
            self.logger.info(f"后设置止损: {stop_loss_price:.2f}, 止盈: {take_profit_price:.2f}")
            ...
            self.logger.info(f"移动止损初始化: {signal_type} 初始价={current_price:.2f}")
            
            # 初始化移动止盈跟踪
            self.trailing_takeprofit_activated = False
            self.dynamic_takeprofit_price = 0
            self.logger.info(f"移动止盈初始化: {signal_type}, 等待激活涨幅{self.config.trailing_takeprofit_activation_pct*100:.1f}%")
    ...
    def calculate_position_size(self, current_price: float, position_pct: float = None) -> float:
        """计算仓位大小
        
        参数:
            current_price: 当前价格
            position_pct: 仓位比例（可选，默认使用config.base_position_size_pct）
        """
        try:
            # 获取账户余额
            ...
            if position_pct is None:
                position_pct = self.config.base_position_size_pct
            
            position_value = usdt_balance * position_pct
            self.logger.info(f"仓位计算: 总余额={usdt_balance} USDT, {position_pct*100:.1f}% = {position_value:.2f} USDT")
    ...
    def check_stop_loss_take_profit(self, current_price: float, atr_value: float = None) -> Optional[str]:
        ...
        # 计算动态止损止盈价格（使用ATR如果可用）
        stop_loss_price, take_profit_price = self.calculate_dynamic_stop_loss_take_profit(
            entry_price, self.position_type, atr_value
        )
        ...
        # 检查移动止盈（如果配置了）
        if self.config.trailing_takeprofit_pct > 0 and self.config.trailing_takeprofit_activation_pct > 0:
            # 检查是否达到移动止盈激活条件
            if not self.trailing_takeprofit_activated:
                if self.position_type == 'long':
                    # 做多：检查是否上涨超过激活百分比
                    if current_price >= entry_price * (1 + self.config.trailing_takeprofit_activation_pct):
                        self.trailing_takeprofit_activated = True
                        self.logger.info(f"移动止盈激活: 当前价={current_price:.2f} ≥ 激活价={entry_price * (1 + self.config.trailing_takeprofit_activation_pct):.2f} (+{self.config.trailing_takeprofit_activation_pct*100:.1f}%)")
                else:
                    # 做空：检查是否下跌超过激活百分比
                    if current_price <= entry_price * (1 - self.config.trailing_takeprofit_activation_pct):
                        self.trailing_takeprofit_activated = True
            ...
            # 如果移动止盈已激活，检查是否触发
            if self.trailing_takeprofit_activated:
                if self.position_type == 'long':
                    # 做多移动止盈：从最高价回撤trailing_takeprofit_pct%
                    # 确保止盈价不低于基础止盈价
                    base_takeprofit_price = entry_price * (1 + self.config.base_takeprofit_pct)
                    dynamic_takeprofit_price = max(self.position_highest_price, base_takeprofit_price) * (1 - self.config.trailing_takeprofit_pct)
                    
                    # 更新动态止盈价格（如果价格创新高）
                    if current_price > self.position_highest_price:
                        self.dynamic_takeprofit_price = dynamic_takeprofit_price
                        self.logger.debug(f"更新做多移动止盈价: 最高价={self.position_highest_price:.2f}, 动态止盈价={self.dynamic_takeprofit_price:.2f}")
                    
                    # 检查是否触发移动止盈
                    if current_price <= self.dynamic_takeprofit_price:
                        self.logger.info(f"触发移动止盈: 最高价={self.position_highest_price:.2f}, 动态止盈价={self.dynamic_takeprofit_price:.2f}, 回撤={self.config.trailing_takeprofit_pct*100:.1f}%")
                        return '移动止盈'
                else:
                    # 做空移动止盈：从最低价反弹trailing_takeprofit_pct%
                    # 确保止盈价不高于基础止盈价
                    base_takeprofit_price = entry_price * (1 - self.config.base_takeprofit_pct)
                    dynamic_takeprofit_price = min(self.position_lowest_price, base_takeprofit_price) * (1 + self.config.trailing_takeprofit_pct)
                    
                    # 更新动态止盈价格（如果价格创新低）
                    if current_price < self.position_lowest_price:
                        self.dynamic_takeprofit_price = dynamic_takeprofit_price
                        self.logger.debug(f"更新做空移动止盈价: 最低价={self.position_lowest_price:.2f}, 动态止盈价={self.dynamic_takeprofit_price:.2f}")
                    
                    # 检查是否触发移动止盈
                    if current_price >= self.dynamic_takeprofit_price:
                        self.logger.info(f"触发移动止盈: 最低价={self.position_lowest_price:.2f}, 动态止盈价={self.dynamic_takeprofit_price:.2f}, 反弹={self.config.trailing_takeprofit_pct*100:.1f}%")
                        return '移动止盈'
    ...
    def send_feishu_notification(self, message: str):
        ...
        try:
            ...
            self.logger.info(f"飞书通知已发送: {message}")
        except Exception as e:
    ...
    def run(self):
        ...
        while self.is_running:
            try:
                ...
                stop_take_reason = self.check_stop_loss_take_profit(current_price, atr_value)
                ...
                # 处理交易信号（通过动态阈值和层级过滤的信号）
                if signal != 'hold' and signal_strength > 0 and position_size_pct > 0:
                    # 检查是否已有持仓
                    ...
                    if self.position is None:
                        ...
                        # 计算仓位大小（使用信号生成器返回的层级仓位比例）
                        position_size = self.calculate_position_size(current_price, position_size_pct)
                        ...
                        if position_size > 0:
                            ...
                            if success:
                                ...
                                self.logger.info(f"开仓成功: {signal} {position_size} {self.config.symbol}, 仓位层级: {position_size_pct*100:.1f}%")
...
def load_config() -> Config:
    ...
    return Config(
        ...
        trailing_takeprofit_activation_pct=float(config.get('Risk', 'trailing_takeprofit_activation_pct', fallback='0.03')),
...
def main():
    """主函数"""
    print("=" * 70)
    print("OKX末日战车策略 - 修复版（解决51010错误）")
    print("基于原优化版修改，修复OKX模拟账户下单问题")
    ...
    logger.info(f"   移动止损: {config.trailing_stop_pct*100:.1f}%")
    logger.info(f"   移动止盈激活: {config.trailing_takeprofit_activation_pct*100:.1f}%")
    logger.info(f"   移动止盈回撤: {config.trailing_takeprofit_pct*100:.1f}%")
