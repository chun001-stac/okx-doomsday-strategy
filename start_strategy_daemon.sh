...
WORKDIR="/root/.openclaw/workspace/freqtrade_workspace_sol"
LOG_DIR="$WORKDIR/logs"
STRATEGY_PID_FILE="$WORKDIR/strategy_pid.txt"
MONITOR_LOG="$LOG_DIR/strategy_monitor_sol.log"
...
send_feishu_notification() {
    ...
    if [ -f "$OPENCLAW_PATH" ]; then
        "$OPENCLAW_PATH" message send \
            ...
            --message "$message" \
            >> "$LOG_DIR/feishu_notification_sol.log" 2>&1 &
...
start_strategy() {
    ...
    # 使用虚拟环境Python
    cd "$WORKDIR"
    nohup .venv/bin/python "$STRATEGY_SCRIPT" > "$LOG_DIR/current_strategy_sol.log" 2>&1 &
    NEW_PID=$!
