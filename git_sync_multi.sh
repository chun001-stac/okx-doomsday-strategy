#!/bin/bash
# 多实例Git同步脚本（安全版本，支持环境变量和配置文件）
# 自动提交所有策略实例的改动到GitHub仓库

set -e

# ==================== GitHub令牌配置（安全方式）====================
# 优先级：环境变量 > 配置文件 > 无令牌（仅本地提交）
# 1. 环境变量方式：export GITHUB_TOKEN=your_token
# 2. 配置文件方式：echo "your_token" > ~/.github_token && chmod 600 ~/.github_token
# =================================================================
...
if [ -z "$GITHUB_TOKEN" ]; then
    ...
    log "⚠️ 未检测到GitHub令牌，仅进行本地提交（不推送远程）"
    log "ℹ️ 设置方式：export GITHUB_TOKEN=your_token 或 echo 'your_token' > ~/.github_token"
else
...
setup_github_remote() {
    ...
    # 只配置ETH实例到GitHub，其他实例仅在本地
    if [ "$INSTANCE_NAME" != "freqtrade_workspace" ]; then
        log "ℹ️ 非ETH实例 ($INSTANCE_NAME)，不配置GitHub远程仓库"
        return 0
    fi
    
    # 检查是否已有远程仓库
    if git remote -v | grep -q "github.com"; then
        log "ℹ️ 已配置GitHub远程仓库"
        return 0
    fi
    
    # 如果没有令牌，不配置远程仓库
    if [ -z "$GITHUB_TOKEN" ]; then
        log "ℹ️ 无GitHub令牌，不配置远程仓库"
        return 0
    fi
    ...
    if [ $? -eq 0 ]; then
        log "✅ 已配置GitHub远程仓库 (实例: $INSTANCE_NAME)"
        
        # 所有实例推送到同一个main分支（代码相同，配置文件被忽略）
        # 如果当前不在main分支，切换到main
...
log "开始多实例Git同步流程（安全版本）..."
log "GitHub仓库: $GITHUB_REPO"
log "令牌状态: $([ -n "$GITHUB_TOKEN" ] && echo "已配置" || echo "未配置，仅本地提交")"
...
自动提交于: $(date)"
        ...
        if git commit -m "$COMMIT_MSG"; then
            ...
            # 如果有远程仓库且是ETH实例，则推送到GitHub
            if [ "$REMOTE_EXISTS" = true ] && [ "$INSTANCE_NAME" = "freqtrade_workspace" ]; then
                # 只有ETH实例推送到GitHub主仓库
                log "🔄 正在推送ETH实例到GitHub仓库..."
                
                # 推送到main分支
                if git push origin main; then
                    log "✅ GitHub推送成功 (分支: main, ETH实例)"
                    ...
                    if git push --force-with-lease origin main; then
                        ...
                        log "ℹ️ 建议手动解决冲突或检查令牌权限"
                    fi
                fi
            else
                # 其他实例不推送到GitHub，只在本地提交
                if [ "$INSTANCE_NAME" != "freqtrade_workspace" ]; then
                    log "ℹ️ 非ETH实例 ($INSTANCE_NAME)，提交仅保存到本地"
...
log "GitHub推送成功次数: $TOTAL_SUCCESS"
log "多实例Git同步流程完成"
echo "" >> "$LOG_FILE"