#!/bin/bash
# 多实例Git同步脚本（所有实例推送到GitHub）
# 自动提交所有策略实例的改动到各自的分支

set -e

# 工作目录列表
INSTANCES=(
    "/root/.openclaw/workspace/freqtrade_workspace"      # ETH
    "/root/.openclaw/workspace/freqtrade_workspace_btc"  # BTC
    "/root/.openclaw/workspace/freqtrade_workspace_sol"  # SOL
    "/root/.openclaw/workspace/freqtrade_workspace_okb"  # DOGE (原OKB，现改为DOGE)
)

LOG_FILE="/root/.openclaw/workspace/freqtrade_workspace/git_sync_multi.log"

# 日志函数
log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$LOG_FILE"
}

# GitHub配置（从环境变量读取令牌）
GITHUB_REPO="chun001-stac/okx-doomsday-strategy"
GITHUB_TOKEN="${GITHUB_TOKEN:-}"

if [ -n "$GITHUB_TOKEN" ]; then
    GITHUB_URL="https://${GITHUB_TOKEN}@github.com/${GITHUB_REPO}.git"
    log "✅ 检测到GitHub令牌，所有实例将推送到GitHub"
else
    GITHUB_URL="https://github.com/${GITHUB_REPO}.git"
    log "⚠️ 未检测到GitHub令牌，仅进行本地提交"
fi

# 检查并更新git版本
check_and_update_git() {
    log "=== Git版本检查 ==="
    CURRENT_VERSION=$(git --version | awk '{print $3}')
    log "当前Git版本: $CURRENT_VERSION"
}

# 配置Git用户信息
setup_git_user() {
    if [ -z "$(git config user.email)" ]; then
        git config user.email "trader@okx-simulation.com"
    fi
    if [ -z "$(git config user.name)" ]; then
        git config user.name "OKX Simulation Trader"
    fi
}

# 配置GitHub远程仓库（所有实例）
setup_github_remote() {
    INSTANCE_NAME=$1
    
    # 如果没有令牌，不配置远程仓库
    if [ -z "$GITHUB_TOKEN" ]; then
        return 0
    fi
    
    # 检查是否已有远程仓库
    if git remote -v | grep -q "github.com"; then
        return 0
    fi
    
    # 移除可能存在的旧远程仓库
    git remote remove origin 2>/dev/null || true
    
    # 添加带令牌的GitHub远程仓库
    if git remote add origin "$GITHUB_URL" 2>/dev/null; then
        log "✅ 已配置GitHub远程仓库 (实例: $INSTANCE_NAME)"
        
        # 为每个实例设置不同的分支名
        case "$INSTANCE_NAME" in
            "freqtrade_workspace")
                BRANCH_NAME="main-eth"
                ;;
            "freqtrade_workspace_btc")
                BRANCH_NAME="main-btc"
                ;;
            "freqtrade_workspace_sol")
                BRANCH_NAME="main-sol"
                ;;
            "freqtrade_workspace_okb")
                BRANCH_NAME="main-doge"
                ;;
            *)
                BRANCH_NAME="main-$INSTANCE_NAME"
                ;;
        esac
        
        # 重命名当前分支
        CURRENT_BRANCH=$(git branch --show-current 2>/dev/null || echo "main")
        if [ "$CURRENT_BRANCH" != "$BRANCH_NAME" ]; then
            git branch -m "$BRANCH_NAME" 2>/dev/null || true
        fi
        
        log "ℹ️ 使用分支: $BRANCH_NAME"
        return 0
    else
        log "❌ 配置GitHub远程仓库失败"
        return 1
    fi
}

log "开始多实例Git同步流程（所有实例推送到GitHub）..."
log "GitHub令牌状态: $([ -n "$GITHUB_TOKEN" ] && echo "已配置" || echo "未配置")"

check_and_update_git

TOTAL_COMMITS=0
TOTAL_SUCCESS=0

for INSTANCE_DIR in "${INSTANCES[@]}"; do
    INSTANCE_NAME=$(basename "$INSTANCE_DIR")
    
    log ""
    log "=== 处理实例: $INSTANCE_NAME ==="
    
    if [ ! -d "$INSTANCE_DIR" ] || [ ! -d "$INSTANCE_DIR/.git" ]; then
        log "跳过: 目录或Git仓库不存在"
        continue
    fi
    
    cd "$INSTANCE_DIR"
    
    setup_git_user
    setup_github_remote "$INSTANCE_NAME"
    
    # 检查是否有远程仓库
    if git remote -v | grep -q "github.com"; then
        REMOTE_EXISTS=true
        log "已连接远程仓库"
    else
        REMOTE_EXISTS=false
        log "仅本地提交"
    fi
    
    # 检查未提交的更改
    CHANGED_FILES=$(git status --porcelain)
    if [ -z "$CHANGED_FILES" ]; then
        log "没有更改，跳过"
        continue
    fi
    
    # 提交更改
    git add -A
    COMMIT_MSG="策略更新 $(date '+%Y-%m-%d %H:%M:%S') - $INSTANCE_NAME"
    if git commit -m "$COMMIT_MSG"; then
        log "✅ 已提交更改"
        TOTAL_COMMITS=$((TOTAL_COMMITS + 1))
        
        # 推送到GitHub（所有实例且有令牌）
        if [ "$REMOTE_EXISTS" = true ] && [ -n "$GITHUB_TOKEN" ]; then
            # 获取当前分支名
            CURRENT_BRANCH=$(git branch --show-current 2>/dev/null || echo "main")
            
            log "🔄 正在推送到GitHub (分支: $CURRENT_BRANCH)"
            
            # 尝试推送，如果失败则强制推送（避免历史冲突）
            if git push origin "$CURRENT_BRANCH" 2>/dev/null; then
                log "✅ GitHub推送成功 (分支: $CURRENT_BRANCH)"
                TOTAL_SUCCESS=$((TOTAL_SUCCESS + 1))
            else
                log "⚠️ 首次推送失败，尝试强制推送..."
                if git push --force-with-lease origin "$CURRENT_BRANCH" 2>/dev/null; then
                    log "✅ GitHub强制推送成功 (分支: $CURRENT_BRANCH)"
                    TOTAL_SUCCESS=$((TOTAL_SUCCESS + 1))
                else
                    log "❌ GitHub推送失败，但提交已保存到本地"
                    log "ℹ️ 可能的原因：令牌权限不足、网络问题或分支冲突"
                fi
            fi
        elif [ -z "$GITHUB_TOKEN" ]; then
            log "ℹ️ 无GitHub令牌，提交仅保存到本地"
        fi
    else
        log "❌ 提交失败"
    fi
done

log ""
log "=== 同步完成 ==="
log "总提交: $TOTAL_COMMITS, 成功推送: $TOTAL_SUCCESS"
log "GitHub分支："
log "  - main-eth: ETH实例"
log "  - main-btc: BTC实例"  
log "  - main-sol: SOL实例"
log "  - main-doge: DOGE实例"